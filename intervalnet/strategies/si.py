...
class SynapticIntelligencePlugin(StrategyPlugin):
    ...
    def before_training_exp(self, strategy: "BaseStrategy", **kwargs):
        ...
        SynapticIntelligencePlugin.create_syn_data(
            strategy.model, self.ewc_data, self.syn_data, self.excluded_parameters
        )

        SynapticIntelligencePlugin.init_batch(strategy.model, self.ewc_data, self.syn_data, self.excluded_parameters)
    ...
    def before_training_iteration(self, strategy: "BaseStrategy", **kwargs):
        ...
        SynapticIntelligencePlugin.pre_update(strategy.model, self.syn_data, self.excluded_parameters)

    def after_training_iteration(self, strategy: "BaseStrategy", **kwargs):
        super().after_training_iteration(strategy, **kwargs)
        SynapticIntelligencePlugin.post_update(strategy.model, self.syn_data, self.excluded_parameters)
    ...
    def create_syn_data(
        ...
        params = SynapticIntelligencePlugin.allowed_parameters(model, excluded_parameters)
    ...
    def pre_update(model, syn_data: SynDataType, excluded_parameters: Set[str]):
        SynapticIntelligencePlugin.extract_weights(model, syn_data["old_theta"], excluded_parameters)

    @staticmethod
    @torch.no_grad()
    def post_update(model, syn_data: SynDataType, excluded_parameters: Set[str]):
        SynapticIntelligencePlugin.extract_weights(model, syn_data["new_theta"], excluded_parameters)
        SynapticIntelligencePlugin.extract_grad(model, syn_data["grad"], excluded_parameters)

        for param_name in syn_data["trajectory"]:
            syn_data["trajectory"][param_name] += syn_data["grad"][param_name] * (
                syn_data["new_theta"][param_name] - syn_data["old_theta"][param_name]
            )
        ...
        for param_name in syn_data["cum_trajectory"]:
            syn_data["cum_trajectory"][param_name] += (
                ...
                * syn_data["trajectory"][param_name]
                / (np.square(syn_data["new_theta"][param_name] - ewc_data[0][param_name]) + eps)
            )
    ...
    def allowed_parameters(model: Module, excluded_parameters: Set[str]) -> List[Tuple[str, Tensor]]:
        ...
        allow_list = SynapticIntelligencePlugin.not_excluded_parameters(model, excluded_parameters)
